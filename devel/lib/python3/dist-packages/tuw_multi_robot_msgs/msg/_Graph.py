...
class Graph(genpy.Message):
  _md5sum = "97d59ea89314d006aa982058da29d9fc"
  ...
  def serialize(self, buff):
    ...
    try:
      ...
      for val1 in self.vertices:
        ...
        length = len(val1.direction)
        buff.write(_struct_I.pack(length))
        for val2 in val1.direction:
          length = len(val2)
          if python3 or type(val2) == unicode:
            val2 = val2.encode('utf-8')
            length = len(val2)
          buff.write(struct.Struct('<I%ss'%length).pack(length, val2))
        _x = val1.special_pose
  ...
  def deserialize(self, str):
    ...
    try:
      ...
      for i in range(0, length):
        ...
        (val1.layout,) = _get_struct_i().unpack(str[start:end])
        start = end
        end += 4
        (length,) = _struct_I.unpack(str[start:end])
        val1.direction = []
        for i in range(0, length):
          ...
          if python3:
            ...
            val2 = str[start:end]
          val1.direction.append(val2)
        start = end
        end += 1
        (val1.special_pose,) = _get_struct_B().unpack(str[start:end])
        val1.special_pose = bool(val1.special_pose)
        start = end
        end += 4
        (length,) = _struct_I.unpack(str[start:end])
        start = end
        ...
        if python3:
          val1.traffic_rule = str[start:end].decode('utf-8', 'rosmsg')
        else:
          val1.traffic_rule = str[start:end]
