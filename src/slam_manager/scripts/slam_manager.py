...
def kill_nodes_and_verify(nodes_to_kill, max_retries=3):
    for attempt in range(max_retries):
        ...
        if not remaining_nodes:
            print_debug("All nodes killed successfully.")
            return True

        for node in remaining_nodes:
            success = kill_rosnode(node)
            if success:
                print_debug(f"Killed node \"{node}\".")
            else:
                print_debug(f"Failed to kill node \"{node}\" (attempt {attempt + 1}).")
    ...
    if still_running:
        print_debug(f"Some nodes could not be killed: {still_running}")
        return False
...
class SlamManager(object):
    ...
    def do_mapping(self):
        ...
        all_nodes = (
            ...
            + self.slam_config_dict["localization"]["static_map_node"]
        )
    ...
    def do_map_static_load(self, map_file):
        ...
        kill_nodes_and_verify(all_nodes)
        print_debug("----------EDITING_MAP----------")

        self.map_publisher_id = ""
        self.current_map = map_file
        launch_static_map_cmd = "{} simulation:={} map_file:={} subfix:={}".format(
            self.slam_config_dict["localization"]["launch_static_map"],
            self.simulation_str,
            map_file,
            self.subfix,
        )
        print_debug(launch_static_map_cmd)
        os.system(launch_static_map_cmd)

    def begin_load_static_map(self, map_file):
        print_debug("Begin load static map: {}".format(map_file))
        # self.load_map_cnt += 1
    ...
    def map_cb(self, msg):
        ...
        if self.map_publisher_id == "":
            print_warn('Map publish id received: {}'.format(msg._connection_header["callerid"]))
        ...
        while not rospy.is_shutdown():
            ...
            # State: SWITCH_MAPPING
                ...
                if rospy.get_time() - begin_mapping_time > self.mapping_timeout:
                    ...
                    print_info("Config MAPPING map publisher: {}".format(self.slam_config_dict["mapping"]["ros_node"][0]))
                    print_info("Map publisher id received: {}".format(self.map_publisher_id))
                    self.main_state = MainState.ERROR_MAPPING
                # if localization_error:
            ...
            # State: INIT_STATIC_MAP
                ...
                self.main_state = MainState.SWITCH_STATIC_MAP
            # State: SWITCH_STATIC_MAP
            elif self.main_state == MainState.SWITCH_STATIC_MAP:
                if (
                    rospy.get_time() - begin_load_static_map_time > 5.0
                    and self.map_publisher_id
                    == self.slam_config_dict["localization"]["static_map_node"][0]
                ):
                    self.main_state = MainState.STATIC_MAP_LOADED
                if rospy.get_time() - begin_load_static_map_time > self.localization_timeout:
                    rospy.logerr("ERROR_LOAD_STATIC_MAP: wait static map timeout")
                    print_info("Config EDITING_MAP map publisher: {}".format(self.slam_config_dict["localization"]["static_map_node"][0]))
                    print_info("Map publisher id received: {}".format(self.map_publisher_id))
            ...
            # State: STATIC_MAP_LOADED
                ...
                if self.cal_load_static_map_time:
                    ...
                    rospy.loginfo(
                        "Switching to STATIC_MAP_LOADED take: {} seconds".format(
                            rospy.get_time() - begin_load_static_map_time
                        )
                    )
                    print_info("STATIC_MAP_LOADED map callerid: {}".format(self.map_publisher_id))
                if self.load_map_request:
                    self.load_map_request = False
                    self.main_state = MainState.INIT_LOCALIZING
                if self.mapping_request:
            ...
            # State: SWITCH_LOCALIZING
                ...
                if rospy.get_time() - begin_load_map_time > self.localization_timeout:
                    ...
                    print_debug("Transform error: {} ".format(self.transform_error))
                    print_info("Config LOCALIZATION map publisher: {}".format(self.slam_config_dict["localization"]["ros_node"][0]))
                    print_info("Map publisher id received: {}".format(self.map_publisher_id))
                    self.main_state = MainState.ERROR_LOCALIZING
                # if localization_error:
                #     rospy.logerr("ERROR_LOCALIZING: TF")
            ...
            # State: MAPPING
                ...
                if self.cal_load_mapping_time:
                    ...
                    print_info(" MAPPING map callerid: {}".format(self.map_publisher_id))
                    rospy.loginfo("---------------------------")
                if self.load_map_request:
                    self.load_map_request = False
                    self.main_state = MainState.INIT_LOCALIZING
            ...
            # State: LOCALIZING
                ...
                if self.cal_localization_time:
                    ...
                    print_info("LOCALIZING map callerid: {}".format(self.map_publisher_id))
                if self.load_map_request:
                    self.load_map_request = False
                    self.main_state = MainState.INIT_LOCALIZING
                if self.load_static_map_request:
