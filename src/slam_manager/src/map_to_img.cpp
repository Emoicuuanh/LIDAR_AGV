...
int main(int argc, char** argv)
{
  ...
  n.param<int32_t>("thresh_occ", thresh_occ, INITIAL_THRESH_OCC);
  ...
  n.param<int>("map_frequency", map_frequency, 5);

  ros::Rate loop_rate(map_frequency);
  ROS_INFO("Init MapAsImageProvider object");
  map_image_provider =
      new MapAsImageProvider(n, thresh_occ, tile_width, tile_height, draw_robot,
                             publish_full_map, publish_map_tile);
  ...
  while (ros::ok())
  {
    ...
    {
      ROS_WARN_THROTTLE(1, "Wait for transform failed: %s", ex.what());
      continue;
    }
    ...
    try
    {
      ...
      // Thay thế new bằng stack allocation
      tf::Matrix3x3 robot_rot_matrix(transform.getRotation());
      robot_rot_matrix.getEulerYPR(yaw, pitch, roll);

      map_image_provider->updateRobotPosition(
          transform.getOrigin().getX(), transform.getOrigin().getY(), yaw);
    }
    catch (const tf::TransformException& ex)
    {
      ROS_WARN_THROTTLE(1, "Could not get transform between '%s' and '%s': %s",
                        parent_frame.c_str(), child_frame.c_str(), ex.what());
      ros::Duration(1.0).sleep();
      continue;
    }

    // Process map publishing
    if (publish_full_map)
    {
