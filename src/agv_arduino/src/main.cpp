...
int RELEASE_STATE = 0;        // 0 = off 1 = on
...
void setup()
{
  ...
  pinMode(BREAK_MOTOR_PIN, OUTPUT);
...
void loop()
{
  ...
  if (digitalRead(RELEASE_PIN))  // RELEASE OFF
  {
    RELEASE_STATE = 1;
    digitalWrite(BREAK_MOTOR_PIN, OUT_OFF);
  }
  else
  {
    if (RELEASE_STATE == 1)
    ...
    {
      digitalWrite(BREAK_MOTOR_PIN, OUT_ON);
    }
  }
  // Auto charging control
  // Serial.println("Auto charging jack connected");
  Serial.println(digitalRead(AUTO_CHARGING_PIN));
  static unsigned long time_auto_off = millis();
  if (digitalRead(AUTO_CHARGING_PIN) == SENSOR_ON)
  {
    time_auto_off = millis();  // liên tục reset khi pin đang ON
    // if (BATTERY_FULL)
    // {
    ...
    if ((millis() > (3000 + t_auto_charging_jack_connect)) && !BATTERY_FULL)
    {
      ...
      else
      {
        ...
        {
          digitalWrite(AUTO_CHARGING_RELAY_PIN, HIGH);
          Serial.println("AUTO_CHARGING_RELAY_PIN == ON , ros not connected");
          AUTO_CHARGING_RELAY_STATUS = true;
        }
        else
        {
          digitalWrite(AUTO_CHARGING_RELAY_PIN, LOW);
          // Serial.println("inChargingRange == 0 , ros not connected");
          AUTO_CHARGING_RELAY_STATUS = false;
        }
      }
    }
    else
    {
      ...
      {
        digitalWrite(AUTO_CHARGING_RELAY_PIN, LOW);
        // Serial.println("Auto charging jack connected but waiting for 3s or battery full, ros not connected");
        AUTO_CHARGING_RELAY_STATUS = false;
      }
    }
  }
  ...
  {
    // time_auto_off được reset liên tục khi pin ON
    // → khi pin OFF, đếm từ lúc pin vừa tắt
    if (millis() - time_auto_off > 3000)
    {
      digitalWrite(AUTO_CHARGING_RELAY_PIN, LOW);
      // Serial.println("khong doc duoc dien ap sac, ngat");
      AUTO_CHARGING_RELAY_STATUS = false;
...
void Lift_Cart(int cmd)
{
  ...
  else if (cmd == LIFT_MIN_POSITION && (!MIN_LIMIT || limitMinHold) &&
           digitalRead(EMG_1_PIN) == SENSOR_ON)
  {
    runLiftMotor(LIFT_DOWN, StartSpeed);
    // Serial.println("MIN");
