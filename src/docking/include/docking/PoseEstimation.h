...
public:
  ...
  void setMaxCorrespondenceDistance(double ICP_max_correspondence_distance)
  {
    ICP_max_correspondence_distance_ = ICP_max_correspondence_distance;
  }
  void setMaxIterations(double ICP_max_iterations)
  {
    ICP_max_iterations_ = ICP_max_iterations;
  }
  void setMaxTransformationEps(double ICP_max_transformation_eps)
  {
    ICP_max_transformation_eps_ = ICP_max_transformation_eps;
  }
  void setMaxRotationEps(double ICP_max_transformation_rotation_eps)
  {
    ICP_max_transformation_rotation_eps_ = ICP_max_transformation_rotation_eps;
  }
  void setMaxEuclideanFitnessEps(double ICP_max_euclidean_fitness_eps)
  {
    ICP_max_euclidean_fitness_eps_ = ICP_max_euclidean_fitness_eps;
  }

  void
  setParams(pcl::IterativeClosestPoint<pcl::PointXYZRGB, pcl::PointXYZRGB>::Ptr
                registrationPtr)
  {
    registrationPtr->setMaximumIterations(ICP_max_iterations_);
    registrationPtr->setMaxCorrespondenceDistance(
    ...
    ROS_INFO_STREAM("ICP-SETPARAMS-- GET Iterations "
                    << registrationPtr->getMaximumIterations());
    ROS_INFO_STREAM("ICP-SETPARAMS-- GET Max Correspondence Distance "
                    << registrationPtr->getMaxCorrespondenceDistance());
    ROS_INFO_STREAM("ICP-SETPARAMS-- GET Max Transformation Epsilon "
                    << registrationPtr->getTransformationEpsilon());
    ROS_INFO_STREAM("ICP-SETPARAMS-- GET Max Euclidean Fitness Epsilon "
                    << registrationPtr->getEuclideanFitnessEpsilon());
  }
  ...
  bool ICP2D(pcl::PointCloud<pcl::PointXYZRGB>::Ptr inputCloudPtr,
             ...
             pcl::IterativeClosestPointNonLinear<
                 pcl::PointXYZRGB, pcl::PointXYZRGB>::Ptr registrationPtr)
  {
    ...
    pcl::registration::WarpPointRigid3D<pcl::PointXYZRGB, pcl::PointXYZRGB>::Ptr
        warp_fcn(new pcl::registration::WarpPointRigid3D<pcl::PointXYZRGB,
                                                         pcl::PointXYZRGB>);

    // Create a TransformationEstimationLM object, and set the warp to it
    pcl::registration::TransformationEstimationLM<pcl::PointXYZRGB,
                                                  pcl::PointXYZRGB>::Ptr
        te(new pcl::registration::TransformationEstimationLM<pcl::PointXYZRGB,
                                                             pcl::PointXYZRGB>);
  ...
  bool ICP(pcl::PointCloud<pcl::PointXYZRGB>::Ptr inputCloudPtr,
           pcl::PointCloud<pcl::PointXYZRGB>::Ptr targetPCLPtr,
           pcl::PointCloud<pcl::PointXYZRGB>::Ptr outCloudPtr,
  ...
  ///////////////// BEGIN clusterICP /////////////////
  void clusterICP(docking::Cluster::Ptr clusterPtr,
                  pcl::PointCloud<pcl::PointXYZRGB>::Ptr targetPCLPtr)
  {

    //          ROS_INFO_STREAM("ICP-CLUS--PROCESSING CLUSTER ID " << clusterPtr->clusterID.data);

    pcl::PointCloud<pcl::PointXYZRGB>::Ptr inCloudPCLPtr(
        new pcl::PointCloud<pcl::PointXYZRGB>());
    pcl::PointCloud<pcl::PointXYZRGB>::Ptr outCloudPCLPtr(
        new pcl::PointCloud<pcl::PointXYZRGB>());
    ...
    pcl::IterativeClosestPointNonLinear<pcl::PointXYZRGB, pcl::PointXYZRGB>::Ptr
        registrationPtr(
            new pcl::IterativeClosestPointNonLinear<pcl::PointXYZRGB,
                                                    pcl::PointXYZRGB>());
    ...
    success =
        ICP2D(inCloudPCLPtr, targetPCLPtr, outCloudPCLPtr, registrationPtr);
    ...
    if (success)
    {
      ...
      ROS_INFO_STREAM("ICP-CLUS-- ICP SCORE CLUSTER ID "
                      << clusterPtr->clusterID.data << " = "
                      << registrationPtr->getFitnessScore());
      ...
      ROS_INFO_STREAM("ICP-CLUS-- ICP  TRANSFORMATION ON CLUSTER ID "
                      << clusterPtr->clusterID.data << "\n"
      ...
      if (clusterPtr->icp.score < ICP_min_score_)
      {
        ...
        clusterPtr->icp.poseTextMarker =
            markPose(clusterPtr->icp.poseStamped.pose);
        ROS_WARN_STREAM("ICP-CLUS-- CLUSTER ID "
                        << clusterPtr->clusterID.data
                        << " POTENTIALLY IDENTIFIED AS DOCK");
      }
    }
    else
    {
      ROS_ERROR_STREAM("ICP-CLUS-- FAILED ICP ON CLUSTER ID "
                       << clusterPtr->clusterID.data);
    }
  }
  ///////////////// END clusterICP /////////////////
  ...
  bool clusterArrayICP(docking::ClusterArray::Ptr clustersPtr,
                       pcl::PointCloud<pcl::PointXYZRGB>::Ptr targetPCLPtr,
                       docking::Cluster::Ptr dockClusterPtr)
  {
    ...
    for (size_t i = 0; i < clustersPtr->clusters.size(); i++)
    {
      ...
      *currentClusterPtr = clustersPtr->clusters.at(i);
      ...
      {
        ROS_INFO_STREAM("ICP-CLUS-ARRAY--SKIPPING ICP FOR CURRENT CLUSTER "
                        << i << " WITH "
                        << currentClusterPtr->lines.lines.size() << " LINES");
        continue;
      }
      ...
      for (size_t j = 0; j < currentClusterPtr->lines.lines.size(); j++)
      {
        ...
        {
          double angle = getAngle(currentClusterPtr->lines.lines.at(j),
                                  currentClusterPtr->lines.lines.at(k));
          ...
          if (angle < 20)
          {
            ...
            ROS_INFO_STREAM("SKIPPING ICP FOR CURRENT CLUSTER "
                            << i
                            << " WITH POSSIBLE PARALLEL LINES OF CIRCULAR "
                               "OBJECT WITH ANGLE "
                            << angle);
      ...
      {
        ROS_WARN_STREAM(
            "ICP-CLUS-ARRAY--DOCK POTENTIALLY IDENTIFIED AT CLUSTER "
            << i << " with score " << currentClusterPtr->icp.score);
        ...
        if (currentClusterPtr->icp.score <=
            clustersPtr->clusters.at(lowestScoreIndex).icp.score)
        {
    ...
    {
      ROS_WARN_STREAM("ICP-CLUS-ARRAY--DOCK SUCCESSFULLY IDENTIFIED AS CLUSTER "
                      << lowestScoreIndex << " !!!!!!!!!!");
      return true;
    }
