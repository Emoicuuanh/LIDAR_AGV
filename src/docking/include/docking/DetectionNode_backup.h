...
#include "docking/DockService.h" // Assumes docking/DockService.srv: geometry_msgs/Pose pose_target, bool enable_detect --- bool success
...
public:
  ...
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr dockTargetPCLPtr_; //Template de so sanh
  ...
  geometry_msgs::TransformStamped base_link_to_leap_motion; // Frames named "base_link" and "leap_motion"
  bool isGotTf = false;
  ...
  void initDockParams()
  {
    ...
    if (dockFilepathExists)
    {
      ...
      if (readPointCloudFile(dockFilePath_, dockTargetPCLPtr) == false) // doc file template
      {
  ...
  bool dockServiceCallback(docking::DockService::Request &req, docking::DockService::Response &res)
  {
    ...
    res.success = true; // Always return success to acknowledge receipt
    ...
    {
      // Transform pose_target to cloud_frame_
      ...
      try
      {
        //transform input pose (map) -> last target pose (laser_frame)
        tfBuffer_.transform(input_pose, last_target_pose_, laser_frame_, ros::Duration(1.0));
      }
  ...
  void detectDockNearPose(const geometry_msgs::PoseStamped &target_pose, docking::ClusterArray::Ptr clustersPtr_)
  {
    ...
    if (icpSuccess)
    {
      ...
      tf2::Transform tf_with_offset = tf_icp * offset;
