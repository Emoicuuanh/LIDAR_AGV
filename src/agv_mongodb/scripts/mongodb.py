...
class mongodb:
    ...
    def getQueueMission(
        ...
        # ==== IN TOÀN BỘ DỮ LIỆU NHẬN TỪ SERVER ====
        rospy.logwarn("[DEBUG] ====== RAW MISSION FROM SERVER ======")
        rospy.logwarn("[DEBUG] Mission name : {}".format(firstMission.get("name", "?")))
        rospy.logwarn("[DEBUG] Mission map  : {}".format(firstMission.get("map", "?")))
        rospy.logwarn("[DEBUG] Total actions: {}".format(size))
        for idx, act in enumerate(firstMission.get("actions", [])):
            rospy.logwarn("[DEBUG] Action[{}]: {}".format(idx, act))
        rospy.logwarn("[DEBUG] ========================================")
        # =============================================
        ...
        for i in range(0, size):
            ...
            if (
                ...
                rospy.logwarn("[DEBUG] getMovingAction result (action {}): {}".format(i, convertedMission["actions"][i]))
                ...
                rospy.logwarn("[DEBUG] getMovingAction result (follow_path action {}): {}".format(i, convertedMission["actions"][i]))
                old_goal = convertedMission["actions"][i]["waypoints"][-1][
                    "name"
                ]
                if convertedMission["actions"][i] == "NO_ROUTE":
