...
class mongodb:
    ...
    def calcWaypoints(self, fromPos, toPos, mapName):
        ...
        name_to_data = {item["name"]: item for item in dataDict}

        ret = []
        prev_to_path_type = None  # Sẽ cập nhật trong vòng lặp

        for idx, name in enumerate(path):
            node = name_to_data[name].copy()

            # Đồng bộ key "data" nếu có
            node["position"] = node.pop("data", node.get("position"))

            # Lấy properties.cubic của điểm hiện tại
            current_props = node["position"].get("properties") or {}
            current_is_cubic = current_props.get("cubic", 0)

            route = None

            if idx < len(path) - 1:
                to_name = path[idx + 1]
                next_points = node["position"].get("next_points", {})
                to_point_data = next_points.get(to_name, {})
                to_path_type = to_point_data.get("path_type")

                # Lấy to_is_cubic từ điểm tiếp theo
                next_node = name_to_data.get(to_name, {})
                next_props = (
                    next_node.get("data", next_node.get("position", {})).get("properties") or {}
                )
                to_is_cubic = next_props.get("cubic", 0)

                # Điểm đầu tiên: from_path_type = to_path_type
                if idx == 0:
                    from_path_type = to_path_type
                else:
                    from_path_type = prev_to_path_type

                route = {
                    "from_point_name": name,
                    "to_point_name": to_name,
                    "from_path_type": from_path_type,
                    "to_path_type": to_path_type,
                    "from_is_cubic": current_is_cubic,
                    "to_is_cubic": to_is_cubic
                }

                if current_is_cubic == 1:
                    route["cubic_start_ctrl"] = to_point_data.get("cubic_start_ctrl")
                    route["cubic_end_ctrl"] = to_point_data.get("cubic_end_ctrl")

                # Cập nhật prev_to_path_type cho lần lặp sau
                prev_to_path_type = to_path_type

            node["position"]["route"] = route
            ret.append(node)
    ...
    #         # #         )
    #         # if (number_of_keys > 1 and "pose_1" in varDict and "pose"):
    #         #     firstMission["name"] = (
    #         #         firstMission["name"] + "_" + varDict["pose"] + "_" + varDict["pose_1"])
    #         # else:
    #         #     firstMission["name"] = (
    #         #         firstMission["name"] + "_" + varDict["pose"])

    #         # Set auto name for mission
    #         number_of_keys = len(varDict)

    #         if number_of_keys > 0:
    #             firstMission["name"] = firstMission["name"] + "_" + "_".join(str(value) for value in varDict.values())
    #         else:
    #             firstMission["name"] = firstMission["name"]  # No change if varDict is empty


    #         print_info("firstMissionName : {}".format(firstMission["name"]))

    #         # Check if mission already exist -> delete and add again to update
    #         listQueue = list(
    ...
    #         if len(listQueue) != 0:
    #             print_info(
    #                 'Mission "{}" already exist -> Update'.format(
    #                     firstMission["name"]
    #                 )
    #             )
    #             mission = listQueue[0]
    ...
    #     # Chuyển dataDict["variables"] thành OrderedDict để giữ nguyên thứ tự
    #     varDict = OrderedDict(dataDict["variables"]) if "variables" in dataDict else None
    #     desc = dataDict["desc"] if "desc" in dataDict else ""
    #     group = dataDict["group"] if "group" in dataDict else ""
    ...
    #         for i in range(size):
    #             actionParamDict = firstMission["actions"][i]["params"]
    #             missionVarDict = firstMission.get("params", {})
    #             if not actionParamDict or not missionVarDict:
    #                 break
    #             for actionParamKey, actionParamValue in actionParamDict.items():
    #                 paramType = type(actionParamValue).__name__
    #                 if paramType == "str" and actionParamValue.startswith("$"):
    #                     isAssign = False
    #                     for varKey, varValue in missionVarDict.items():
    #                         if actionParamValue[1:] == varKey:
    #                             actionParamDict[actionParamKey] = varDict[actionParamValue[1:]]
    #                             isAssign = True
    #                             break
    #                     if not isAssign:
    #                         print_error("Missing param: {}".format(actionParamValue))
    #         del firstMission["_id"]
    #         if "params" in firstMission:
    #             del firstMission["params"]
    #         firstMission["desc"] = desc
    ...
    #         number_of_keys = len(varDict)
    #         if number_of_keys > 0:
    #             # Sử dụng OrderedDict để đảm bảo thứ tự các giá trị
    #             firstMission["name"] = (
    #                 firstMission["name"] + "_" + "_".join(str(value) for value in varDict.values())
    #             )
    #         else:
    #             firstMission["name"] = firstMission["name"]  # No change if varDict is empty
    ...
    #         # Check if mission already exists -> delete and add again to update
    #         listQueue = list(
    #             self.missionCollection.find(
    #                 {"name": firstMission["name"], "map": curMap}
    #             )
    ...
    def makeMissionWithVar(self, missionName, curMap, dataDict):
        ...
        # Convert to OrderedDict to maintain input order (important fallback)
        varDict = OrderedDict(dataDict["variables"]) if "variables" in dataDict else None
        desc = dataDict.get("desc", "")
        ...
        self.listMission = list(
            self.missionCollection.find({
                ...
                "$or": [
                    {"map": curMap},
                    {"is_mission_base": True}
                ]
            })
        )

        if not self.listMission:
            print_error(f"Mission '{missionName}' on map '{curMap}' not found.")
            return

        firstMission = self.listMission[0]
        size = len(firstMission["actions"])

        # Replace variables in mission actions
        if varDict is not None:
            for i in range(size):
                actionParamDict = firstMission["actions"][i]["params"]
                missionVarDict = firstMission.get("params", {})
                if not actionParamDict or not missionVarDict:
                    break
                for actionParamKey, actionParamValue in actionParamDict.items():
                    if isinstance(actionParamValue, str) and actionParamValue.startswith("$"):
                        var_name = actionParamValue[1:]
                        if var_name in missionVarDict and var_name in varDict:
                            actionParamDict[actionParamKey] = varDict[var_name]
                        else:
                            print_error("Missing param: {}".format(actionParamValue))

            # Clean up and assign metadata
            firstMission.pop("_id", None)
            firstMission.pop("params", None)
            firstMission["desc"] = desc
            firstMission["group"] = group
            firstMission["map"] = curMap

            # Auto-generate name based on sorted POSE keys
            pose_keys = [k for k in varDict if k.startswith("POSE_")]
            if pose_keys:
                sorted_poses = sorted(pose_keys, key=lambda k: int(k.split("_")[1]))
                pose_values = [str(varDict[k]) for k in sorted_poses]
                firstMission["name"] = firstMission["name"] + "_" + "_".join(pose_values)
            else:
                # fallback: use the order in varDict (via OrderedDict)
                firstMission["name"] = firstMission["name"] + "_" + "_".join(str(v) for v in varDict.values())

            print_info("firstMissionName : {}".format(firstMission["name"]))

            # Replace existing mission if exists
            listQueue = list(
                self.missionCollection.find(
                    {"name": firstMission["name"], "map": curMap}
                )
            ...
            if listQueue:
                print_info('Mission "{}" already exists -> Updating'.format(firstMission["name"]))
                mission = listQueue[0]
                self.missionCollection.delete_one({"_id": mission["_id"]})

            self.missionCollection.insert_one(firstMission)

        # Enqueue mission to run
        missionQueue = {
            "name": firstMission["name"],
            "desc": firstMission["desc"],
            "time": datetime.datetime.now(),
    ...
    def getQueueMission(
        ...
        for i in range(0, size):
            ...
            if (
                ...
                if self.curPos == "Not defined" and checkRobotInPoseWhenStart:
    ...
    def downloadFile(self, fileName, filePath, type, contentType=""):
        ...
        Args:
            ...
            contentType (String, optional): Content type of file in mongodb server ("png", "pbstream", ...). Defaults to "".
                Phải có contentType vì nếu type = "map" thì có thể có 2 file png và pbstream
        """
        file = None
        gridFile = None
        try:
            if contentType == "":
                gridFile = self.fs.find({"filename": fileName, "type": type})
            else:
                gridFile = self.fs.find(
                    {
                        "filename": fileName,
                        "type": type,
            ...
            for grid_data in gridFile:
                fileID = grid_data._id
                file = open(filePath, "wb")
                self.fs.download_to_stream(fileID, file)
                file.close()
                print_info(
                    'Downloaded file: "{}" to {}'.format(fileName, filePath)
                )
        except Exception as e:
            if file != None:
                file.close()
    ...
    def downloadYamlMapFile(self, mapName, folderPath, imgFileType):
        ...
        try:
            ...
            print_debug("Download YAML file: {}".format(mapName))
    ...
    def downloadMapImage(self, mapName, folderPath, imgYamlFileType):
        ...
        self.downloadFile(mapName, filePngPath, "map", "png")
