...
def dict_to_pose(input_dict):
    pose_ = Pose()
    print("INput dict:")
    print(input_dict["orientation"])
    pose_.position.x = input_dict["position"]["x"]
    pose_.position.y= input_dict["position"]["y"]
    if input_dict["orientation"] is not None:
        pose_.orientation.x = input_dict["orientation"]["x"]
        pose_.orientation.y = input_dict["orientation"]["y"]
        pose_.orientation.z = input_dict["orientation"]["z"]
        pose_.orientation.w = input_dict["orientation"]["w"]
...
def lockup_pose(tf_listener, base_frame, robot_frame):
    try:
        tf_listener.waitForTransform(
            base_frame, robot_frame, rospy.Time(0), rospy.Duration(1.0)
        )
