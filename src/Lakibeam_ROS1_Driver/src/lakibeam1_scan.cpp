...
int main(int argc, char** argv)
{
  ...
  std::string hostip, sensorip, port, frame_id, output_topic, scanfreq, filter,
      laser_enable, scan_range_start, scan_range_stop, filter_threshold,
      gain_factor;
  ...
  nh.getParam("filter_threshold", filter_threshold);
  nh.getParam("gain_factor", gain_factor);

  ros::Publisher scan_pub =
      nh.advertise<sensor_msgs::LaserScan>(output_topic, 1000);
  ...
  setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (char*)&mw_optval,
             sizeof(mw_optval));
    ...
    if (scan_vec_ready == 0)
    {
      while (1)
      {
        if (j == 12)
        {
          ...
          {
            resolution =
                (MSOP_Data.BlockID[1].Azimuth - MSOP_Data.BlockID[0].Azimuth) /
                16;
          }
          j = 0;
        }
