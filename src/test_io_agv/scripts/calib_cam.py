...
class CALIB_CAM(object):
    ...
    def calibrate_position(self, save_path="/tmp/offset_position.yaml"):
        ...
        # Tính offset - Camera lệch so với tâm AGV
        # Nếu camera ở tâm: init_dis + after_dis = 0 (vì xoay 180°)
        # Nếu lệch: tổng ≠ 0 → offset = tổng/2
        offset_x = (init_dis_x + after_dis_x) / 2.0
        offset_y = (init_dis_y + after_dis_y) / 2.0
    ...
    def calibrate_position_with_creep_compensation(self, save_path="/tmp/offset_position_compensated.yaml", camera_angle_offset=None):
        """
        Calibrate position with forward creep compensation
        AGV may move forward 1-2mm during rotation due to mechanical issues
        
        Hệ tọa độ camera lệch 89.3° so với AGV (từ TF: ROTATE_Z_CAM_QR=1.558):
        - +X camera ≈ +Y AGV
        - +Y camera ≈ -X AGV
        Cần chuyển đổi creep từ hệ AGV về hệ camera
        
        Args:
            camera_angle_offset: Góc lệch của camera so với AGV (rad).
                                 Nếu None, dùng giá trị TF: 1.558 rad (89.3°).
                                 Hoặc lấy từ kết quả calibrate_angle().
        """
        rospy.loginfo("BEGIN calibrate_position WITH forward creep compensation")
        
        # Nếu không cung cấp góc lệch, dùng giá trị từ TF tree
        if camera_angle_offset is None:
            camera_angle_offset = 1.57  # 89.3° (từ ROTATE_Z_CAM_QR trong TF)
            rospy.logwarn("Camera angle offset not provided, using TF value: 1.558 rad (89.3°)")
        self.reset_data()
        while not rospy.is_shutdown() and (self.qr_angle is None or self.pose_odom2robot is None):
            rospy.sleep(0.1)
        ...
        # Save initial states
        init_pose = self.pose_odom2robot
        init_odom_x = init_pose.position.x
        init_odom_y = init_pose.position.y
        init_yaw = self.get_yaw(init_pose)
        init_qr_x = self.dis_qr_x
        init_qr_y = self.dis_qr_y
        rospy.loginfo(f"Initial: QR x={init_qr_x*1000:.1f}mm, y={init_qr_y*1000:.1f}mm, yaw={math.degrees(init_yaw):.1f}°")
        ...
        while not rospy.is_shutdown():
            current_yaw = self.get_yaw(self.pose_odom2robot)
            yaw_diff = self.angle_diff(current_yaw, init_yaw)
            if abs(yaw_diff) >= math.radians(179):
                break
            self.cmd_vel_pub.publish(twist)
            rate.sleep()
        ...
        # Read after rotation
        after_pose = self.pose_odom2robot
        after_odom_x = after_pose.position.x
        after_odom_y = after_pose.position.y
        after_yaw = self.get_yaw(after_pose)
        after_qr_x = self.dis_qr_x
        after_qr_y = self.dis_qr_y
        rospy.loginfo(f"After rotation: QR x={after_qr_x*1000:.1f}mm, y={after_qr_y*1000:.1f}mm, yaw={math.degrees(after_yaw):.1f}°")

        # Calculate creep in ODOM frame (AGV frame)
        dx_odom = after_odom_x - init_odom_x
        dy_odom = after_odom_y - init_odom_y
        creep_magnitude = math.sqrt(dx_odom**2 + dy_odom**2)
        
        # Chuyển đổi creep về hệ camera TRƯỚC khi xoay
        # Camera lệch 89.3° so với AGV (góc lệch = camera_angle_offset = 1.558 rad)
        # +X camera ≈ +Y AGV
        # +Y camera ≈ -X AGV
        # Khi AGV xoay, creep được tính trong hệ AGV ban đầu
        # Cần chiếu creep về hệ camera ban đầu
        
        # Xoay vector creep về hệ camera
        # Frame camera = Frame AGV xoay (camera_angle_offset) = xoay 89.3°
        camera_angle = init_yaw + camera_angle_offset
        creep_x_camera = dx_odom * math.cos(camera_angle) + dy_odom * math.sin(camera_angle)
        creep_y_camera = -dx_odom * math.sin(camera_angle) + dy_odom * math.cos(camera_angle)
        
        rospy.loginfo(f"Creep in odom: dx={dx_odom*1000:.2f}mm, dy={dy_odom*1000:.2f}mm")
        rospy.loginfo(f"Creep magnitude: {creep_magnitude*1000:.2f}mm")
        rospy.loginfo(f"Creep in camera frame: x={creep_x_camera*1000:.2f}mm, y={creep_y_camera*1000:.2f}mm")

        # Calculate offset WITHOUT compensation (for comparison)
        offset_x_raw = (init_qr_x + after_qr_x) / 2.0
        offset_y_raw = (init_qr_y + after_qr_y) / 2.0

        # Compensate creep in camera frame
        # Khi AGV xoay + creep, vị trí QR so với camera thay đổi
        # Cần bù creep vào after_qr để có vị trí "như thể AGV xoay tại chỗ"
        # after_qr và creep_x_camera đều đơn vị mét → KHÔNG nhân 1000
        corrected_after_qr_x = after_qr_x + creep_x_camera
        corrected_after_qr_y = after_qr_y + creep_y_camera
        
        offset_x = (init_qr_x + corrected_after_qr_x) / 2.0
        offset_y = (init_qr_y + corrected_after_qr_y) / 2.0
        ...
        data_to_save = {
            "before": {
                "qr_x": float(init_qr_x),
                "qr_y": float(init_qr_y),
                "odom": {"x": float(init_odom_x), "y": float(init_odom_y), "yaw": float(init_yaw)}
            },
            "after": {
                "qr_x": float(after_qr_x),
                "qr_y": float(after_qr_y),
                "odom": {"x": float(after_odom_x), "y": float(after_odom_y), "yaw": float(after_yaw)}
            },
            "creep_odom_mm": {"dx": float(dx_odom * 1000), "dy": float(dy_odom * 1000)},
            "creep_camera_mm": {"x": float(creep_x_camera * 1000), "y": float(creep_y_camera * 1000)},
            "creep_magnitude_mm": float(creep_magnitude * 1000),
            "offset_raw_mm": {"x": float(offset_x_raw * 1000), "y": float(offset_y_raw * 1000)},
            "offset_compensated_mm": {"x": float(offset_x * 1000), "y": float(offset_y * 1000)}
        }

        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        with open(save_path, "w") as f:  # Ghi đè thay vì append
            yaml.dump(data_to_save, f)

        rospy.loginfo(f"Saved to {save_path}")
        rospy.loginfo(f"Offset (raw): x={offset_x_raw*1000:.2f}mm, y={offset_y_raw*1000:.2f}mm")
        rospy.loginfo(f"Offset (compensated): x={offset_x*1000:.2f}mm, y={offset_y*1000:.2f}mm")
        return offset_x, offset_y

    def loop(self):
        while not rospy.is_shutdown():
            user_input = input("Chọn hàm offset [1=position, 2=angle, 3=position+creep_comp, q=quit]: ")
            ...
            if user_input == "1":
                ...
                rospy.loginfo("Running calibrate_position with creep compensation...")
                save_file = os.path.join(results_path, "offset_position_compensated.yaml")
                self.calibrate_position_with_creep_compensation(save_path=save_file)
            else:
                rospy.logwarn("Invalid input. Please enter 1, 2, 3 or q.")
