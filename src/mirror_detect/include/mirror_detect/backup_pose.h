...
//          std::cout << registrationPtr->getFinalTransformation() << std::endl;
  ...
  ///////////////// BEGIN clusterICP /////////////////
  void clusterICP(mirror_detect::Cluster::Ptr clusterPtr, pcl::PointCloud<pcl::PointXYZRGB>::Ptr targetPCLPtr){
...
//        ROS_INFO_STREAM("ICP-CLUS-- INVERTING getFinalTransformation ICP ON CLUSTER ID " << clusterPtr->clusterID.data << "\n" << transformation);
  ...
  bool clusterArrayICP(mirror_detect::ClusterArray::Ptr clustersPtr, pcl::PointCloud<pcl::PointXYZRGB>::Ptr targetPCLPtr, mirror_detect::Cluster::Ptr dockClusterPtr,std::vector<mirror_detect::Cluster::Ptr> &potential,std::vector<float> &score)
  {
    ...
    for(size_t i=0; i<clustersPtr->clusters.size();i++)
    {
      ...
      mirror_detect::Cluster::Ptr currentClusterPtr(new mirror_detect::Cluster());
...
//      ROS_INFO_STREAM("CURRENT LOWEST SCORE VAL " << lowestScore);
      ...
      if(currentClusterPtr->potentialDock.data){
        ...
        ROS_INFO("found dock %ld",i);
        potentialIndex.push_back(i);
        score.push_back(currentClusterPtr->icp.score);
