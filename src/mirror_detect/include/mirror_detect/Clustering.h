...
public:
  ...
  visualization_msgs::Marker markCluster(mirror_detect::Cluster cluster) {
    uint32_t shape = visualization_msgs::Marker::CUBE;
    visualization_msgs::Marker marker;
    marker.header = header_;
    marker.ns = "mirror_detect";
    marker.id = 0;
    marker.type = shape;
...
mirror_detect::Cluster rosifyClusterIntensity(pcl::PointCloud<pcl::PointXYZI>::Ptr &cloudPCLPtr,pcl::PointIndices::Ptr &indicesPCLPtr) {

  mirror_detect::Cluster clusterMsg;
  ...
  // clusterMsg.bbox = getBoundingBoxClusterOrientedIntensity(cloudPCLPtr);
  // clusterMsg.bbox.marker = markCluster(clusterMsg);

  return clusterMsg;
}
///////////////// BEGIN ROSIFY CLUSTER /////////////////
mirror_detect::Cluster rosifyCluster(pcl::PointCloud<pcl::PointXYZRGB>::Ptr &cloudPCLPtr,
...
/// \return bbox
///
mirror_detect::BoundingBox getBoundingBoxCluster(mirror_detect::Cluster cluster) {
...
/// \return bbox
///
mirror_detect::BoundingBox getBoundingBoxClusterOriented(
...
//    printDebugCloud(cloudPointsProjectedXYZPtr);
  ...
  mirror_detect::BoundingBox bboxMsg;
...
/// \return
///
void ClusterPointsIntensity(pcl::PointCloud<pcl::PointXYZI>::Ptr inCloud, mirror_detect::ClusterArray::Ptr clustersPtr, double clusterTolerance, int minSize, int maxSize)
{
  mirror_detect::ClusterArray clusters;
  pcl::PointCloud<pcl::PointXYZI> combinedClustersCloud;
...
//    ROS_WARN_STREAM("CLUSTERING: NO CLUSTERS FOUND ");
  ...
  std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> clustersPCLVector;
  int i = 0;
  for (std::vector<pcl::PointIndices>::const_iterator it = cluster_indices.begin(); it != cluster_indices.end(); ++it, ++i) {
    pcl::PointCloud<pcl::PointXYZI>::Ptr cloudClusterPtr(new pcl::PointCloud<pcl::PointXYZI>);
...
//      ROS_INFO_STREAM("CLUSTERING: Cluster " << i << " has "<< cloudClusterPtr->points.size() << " points");
    ...
    mirror_detect::Cluster clusterMsg = rosifyClusterIntensity(cloudClusterPtr, currentPointIndicesPtr);
...
void ClusterPoints(pcl::PointCloud<pcl::PointXYZRGB>::Ptr inCloud, mirror_detect::ClusterArray::Ptr clustersPtr, double clusterTolerance, int minSize, int maxSize) {
  //std::cout << std::endl;
  // ROS_INFO_STREAM();
...
//      ROS_INFO_STREAM("CLUSTERING: Cluster " << i << " has "<< cloudClusterPtr->points.size() << " points");
    ...
    pcl::PointIndicesPtr currentPointIndicesPtr(new pcl::PointIndices(*it));
    // ROS_INFO_STREAM("ROSIFYING CLUSTER");
    mirror_detect::Cluster clusterMsg = rosifyCluster(cloudClusterPtr, currentPointIndicesPtr);
    clusterMsg.clusterID.data = i;
    clusterMsg.header = clusterMsg.cloud.header = clusterMsg.points.header = clustersPtr->header;
