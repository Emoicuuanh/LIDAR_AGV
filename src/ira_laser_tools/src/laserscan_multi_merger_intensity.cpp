...
public:
  ...
  void reconfigureCallback(laserscan_multi_mergerConfig& config,
                           uint32_t level);
...
private:
  ...
  ros::Publisher laser_scan_publisher_map;
...
void LaserscanMerger::laserscan_topic_parser()
{
  ...
  copy(istream_iterator<string>(iss), istream_iterator<string>(),
       inserter<set<string>>(tokens, tokens.begin()));
  vector<string> tmp_input_topics;

  while (!tokens.empty())
  {
    ROS_INFO("Waiting for topics ...");
    ROS_INFO("Field name:12");
...
LaserscanMerger::LaserscanMerger()
{
  ...
  nh.param<std::string>("cloud_destination_topic", cloud_destination_topic,
                        "/merged_cloud");
  nh.param<std::string>("scan_destination_topic", scan_destination_topic,
                        "/scan_multi");
  ...
  point_cloud_publisher_ = node_.advertise<sensor_msgs::PointCloud2>(
      cloud_destination_topic.c_str(), 1, false);
  laser_scan_publisher_ = node_.advertise<sensor_msgs::LaserScan>(
      scan_destination_topic.c_str(), 1, false);
  laser_scan_publisher_map =
      node_.advertise<sensor_msgs::LaserScan>("scan_map", 1, false);
}
                                   ...
                                   std::string topic)
{
  ...
  try
  {
    ...
    tfListener_.waitForTransform(scan->header.frame_id.c_str(),
                                 destination_frame.c_str(), scan->header.stamp,
                                 ros::Duration(1));
    projector_.transformLaserScanToPointCloud(
        scan->header.frame_id, *scan, tmpCloud1, tfListener_,
        (laser_geometry::channel_option::Distance |
         laser_geometry::channel_option::Intensity));
    tfListener_.transformPointCloud(destination_frame.c_str(), tmpCloud1,
                                    tmpCloud2);
  }
  ...
  if (totalClouds == clouds_modified.size())
  {
    ...
    pcl::PCLPointCloud2 _pclPointCloud2 = clouds[0];
    pcl_conversions::fromPCL(_pclPointCloud2, merged_cloud);
    // merged_cloud=
    ...
    {
      sensor_msgs::PointCloud2 merged_cloud_now;
      _pclPointCloud2 = clouds[i];
      pcl_conversions::fromPCL(_pclPointCloud2, merged_cloud_now);
      pcl::concatenatePointCloud(merged_cloud, merged_cloud_now, merged_cloud);

      clouds_modified[i] = false;
    }

    pcl::PCLPointCloud2 merge_cloud_pcl;
    pcl_conversions::toPCL(merged_cloud, merge_cloud_pcl);
    for (auto& field : merge_cloud_pcl.fields)
    {
      ...
      if (field.name == "intensities")
      {
        ...
        // ROS_INFO("field_name:%s",field.name.c_str());
      }
    }
    point_cloud_publisher_.publish(merged_cloud);

    // pcl::PCLPointCloud2 _pclPointCloud2=merge_cloud_pcl;
    Eigen::MatrixXf points;
    pcl::getPointCloudAsEigen(merge_cloud_pcl, points);

    pointcloud_to_laserscan(points, merge_cloud_pcl);
  }
}
                                              ...
                                              pcl::PCLPointCloud2 merged_cloud)
{
  ...
  pcl::fromPCLPointCloud2(merged_cloud, *cloud2);

  // pcl::fromPCLPointCloud2(merged_cloud, *cloud2);
  uint32_t ranges_size = std::ceil((output->angle_max - output->angle_min) /
                                   output->angle_increment);
  output->ranges.assign(ranges_size, output->range_max + 1.0);
  output->intensities.assign(ranges_size, 0.0);  // Add intensity field
  output->time_increment = (this->scan_time / ranges_size)/2.0;
  // ROS_INFO("time_incre:%f", output->time_increment);
  for (int i = 0; i < points.cols(); i++)
  {
    ...
    if (output->ranges[index] * output->ranges[index] > range_sq)
    {
      ...
      output->intensities[index] = cloud2->points[i].intensity;
    }
  }

  laser_scan_publisher_.publish(output);
  // laser_scan_publisher_map.publish(output);
}

int main(int argc, char** argv)
{
  ros::init(argc, argv, "laser_multi_merger_intensity");
